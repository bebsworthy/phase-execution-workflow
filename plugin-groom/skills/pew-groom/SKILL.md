...
The synthesizer adapts its output format based on issue complexity:

| Mode | Trigger | Sections Included |
|------|---------|-------------------|
| **Full** | M/L/XL complexity, or multiple approaches | All sections (Executive Summary through Definition of Done) |
| **Compact** | XS/S complexity AND single approach | Executive Summary, Effort Estimate (single line), Clarifying Questions (if any), Definition of Done checklist |

Compact mode produces a shorter document appropriate for trivial/small changes. The full technical plan tables, blocker tables, and detailed test strategy are omitted — the information still exists in the intermediate files (05-09) for anyone who needs deeper detail.
...
Clarifying questions generated by the spec evaluator must carry a severity tag so the PO can triage responses:

| Tag | Label | Definition | Implication |
|-----|-------|-----------|-------------|
| `[BLOCKER]` | Blocking | Cannot start implementation without this answer | Work should not begin until PO responds |
| `[IMPORTANT]` | Important | Can start but risk significant rework if assumption is wrong | Start with documented assumption, flag for early PO response |
| `[NICE-TO-HAVE]` | Optional | Would improve quality but safe to assume a reasonable default | Proceed with assumption, address if PO responds |
...
│       │   ├── 08-test-plan.md
│       │   ├── 09-estimation.md
│       │   ├── 10-review-completeness.md  # fast-path: omitted
│       │   ├── 11-review-feasibility.md  # fast-path: omitted
...
| New comments only | Focused re-analysis: intake extracts new comments and cross-references against previous clarifying questions for resolution detection. Phase 3-6 agents instructed to address new information and update analysis |
