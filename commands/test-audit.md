...
Create the `test-review/` directory. This is the shared workspace all agents will write to.
...
> Detect tautological tests — tests that mirror implementation logic rather than independently verifying behavior. Read the inventory at `test-review/01-inventory.json` for file locations. Check for: direct logic mirroring, mock-setup-as-assertion, fixture echo, snapshot tautology. Save findings to `test-review/02-tautological.md`. $ARGUMENTS

### Spawn `test-audit-mocking`
> Detect over-mocking and mock misuse. Read the inventory at `test-review/01-inventory.json`. Check for: testing mock behavior, everything-mocked isolation, mocking what you own, mock depth > 1, mock return values duplicating production logic, missing contract verification. Produce a mock heat map. Save findings to `test-review/03-mocking.md`. $ARGUMENTS

### Spawn `test-audit-framework`
> Detect tests that test the framework or language runtime rather than application logic. Read the inventory at `test-review/01-inventory.json`. Check for: framework behavior tests, language feature tests, trivial getter/setter tests, config-only tests, assertion-free tests (secret catchers), dodger tests. Save findings to `test-review/04-framework.md`. $ARGUMENTS

### Spawn `test-audit-coverage`
> Find missing test coverage and happy-path bias. Read the inventory at `test-review/01-inventory.json`. For each source file with tests, check: error path coverage, boundary conditions, negative/security tests, state transition coverage, integration boundary tests. Prioritize by business criticality. Save findings to `test-review/05-coverage.md`. $ARGUMENTS

### Spawn `test-audit-maintainability`
> Audit test maintainability and structural quality. Read the inventory at `test-review/01-inventory.json`. Check for: implementation coupling, excessive setup, test interdependence, copy-paste proliferation, poor naming, snapshot overuse, test-only production code. Produce a maintainability scorecard. Save findings to `test-review/06-maintainability.md`. $ARGUMENTS

### Spawn `test-audit-flaky`
> Detect flaky tests and CI reliability risks. Read the inventory at `test-review/01-inventory.json`. Check for: timing dependencies, port/network dependencies, file system dependencies, shared mutable state, order-dependent tests, environment assumptions, non-deterministic assertions. Save findings to `test-review/07-flaky.md`. $ARGUMENTS

**Wait for ALL 6 agents to complete.** Verify `test-review/02-tautological.md` through `test-review/07-flaky.md` all exist and are non-empty.

## Step 3 — Phase 3: Synthesis (Sequential)

### Spawn `test-audit-synthesis`
> Consolidate findings from all 6 audit agents into a unified, prioritized remediation roadmap. Read all files in `test-review/` (01 through 07). Deduplicate findings, classify every test (KEEP/REFACTOR/DELETE/REWRITE/MISSING), prioritize into remediation tiers, and produce key metrics. Save to `test-review/08-synthesis.md`. $ARGUMENTS

**Wait for completion.** Verify `test-review/08-synthesis.md` exists and contains: executive summary, metrics table, tiered roadmap, per-file action list, risk assessment.

## Step 4 — Phase 4: Remediation (Sequential)

### Spawn `test-audit-remediation`
> Execute the remediation plan from `test-review/08-synthesis.md`. For each test marked DELETE/REWRITE/REFACTOR, produce concrete code changes with before/after examples. For MISSING tests, write new test code following the test generation rules. Save to `test-review/09-remediation.md`. $ARGUMENTS

**Wait for completion.** Verify `test-review/09-remediation.md` exists and contains: deletions with justification, rewrites with before/after, refactors with before/after, new tests with regression rationale.

## Step 5 — Phase 5: Architecture (Sequential)

### Spawn `test-audit-architecture`
> Design an optimal test directory structure and produce a testing playbook for the project. Read all files in `test-review/`. Propose test organization, coverage thresholds, CI configuration, and a comprehensive TESTING_PLAYBOOK.md with standards for future test writing (by humans and LLM agents). Save to `test-review/10-architecture.md`. $ARGUMENTS

**Wait for completion.** Verify `test-review/10-architecture.md` exists and contains: proposed directory structure, test runner configuration, testing playbook with all sections.

## Step 6 — Read All Phase Files & Synthesize Playbook

After all 10 agents complete, read each file in order:
1. `test-review/01-inventory.json`
2. `test-review/02-tautological.md` through `test-review/07-flaky.md`
3. `test-review/08-synthesis.md`
4. `test-review/09-remediation.md`
5. `test-review/10-architecture.md`

Write **`test-review/playbook.md`** — the final deliverable. Must include:
...
```
[ORCHESTRATOR] PLAYBOOK COMPLETE ✓ — saved to test-review/playbook.md
...
- The `test-review/` directory must contain all 11 files when done.
