...
Write `{output_dir}/05-coverage.md`. For each source file, report missing test categories with business risk and suggested tests. Prioritize by business criticality: payment, auth, data integrity first.

Signal completion: `[test-audit-coverage] COMPLETE ✓ — saved to {output_dir}/05-coverage.md`
