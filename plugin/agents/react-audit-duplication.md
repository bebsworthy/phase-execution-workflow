...
Read `{config.paths.audit_react}/01-inventory.json` for the file inventory, then read the source files.
...
Write `{config.paths.audit_react}/04-duplication.md` using the finding report format from the react-audit skill.
...
Signal completion: `[react-audit-duplication] COMPLETE ✓ -- saved to {config.paths.audit_react}/04-duplication.md`
