...
Read `{config.paths.audit_react}/01-inventory.json` for the stack info, file inventory, and dependency analysis. Then read the source files.
...
Write `{config.paths.audit_react}/06-debt.md` using the finding report format from the react-audit skill.
...
Signal completion: `[react-audit-debt] COMPLETE ✓ -- saved to {config.paths.audit_react}/06-debt.md`
