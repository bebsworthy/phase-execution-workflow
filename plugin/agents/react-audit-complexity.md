...
Read `{config.paths.audit_react}/01-inventory.json` for the file inventory and complexity baseline, then read the source files. Focus on the files flagged as largest, most-imported, and deepest-nested.
...
Write `{config.paths.audit_react}/05-complexity.md` using the finding report format from the react-audit skill.
...
Signal completion: `[react-audit-complexity] COMPLETE ✓ -- saved to {config.paths.audit_react}/05-complexity.md`
